...
export default function Sidebar(props) {
  ...
  return (
    <Drawer classes={{ paper: styles.drawerPaper }} className={styles.drawer} onClose={props.onClose ? props.onClose : null} open={props.open} variant={props.variant}>
      <SidebarAvatar />
      <Divider />
      <SidebarList icon={<DashboardIcon />} text="Dashboard" link="/" title="Reportes" variant="h3" />
      <Grid>
        <List subheader={<ListSubheader className={styles.colored}>Administración</ListSubheader>}>
          <SidebarNestedList icon={<PeopleIcon />} collapseText="Lista de clientes" text="Clientes" link="/clientes" />
          <SidebarNestedList icon={<CartIcon />} collapseText="Lista de productos" text="Productos" extra={true} extraCollapseText="Crear un producto" link="/productos" extraLink="/productos/crear" />
          <SidebarNestedList icon={<FolderIcon />} collapseText="Lista de pedidos" text="Pedidos" link="/pedidos" />
          <SidebarNestedList icon={<RecieptIcon />} collapseText="Lista de facturas" text="Facturas" link="/facturas" />
        </List>
      </Grid>
    </Drawer>
  );
